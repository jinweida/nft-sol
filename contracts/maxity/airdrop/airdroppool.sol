...
contract AirdropPool is Ownable {
    ...
    // The block number when bonus mining end;
    ...
    function newAirdrop(uint256 _tokenAmount, uint256 _newPerBlock, uint256 _startBlock) public onlyOwner {
        require(block.number > endBlock && _startBlock >= endBlock, "Not finished");
        massUpdatePools();
        uint256 beforeAmount = IERC20(bonusToken).balanceOf(address(this));
        TransferHelper.safeTransferFrom(bonusToken, msg.sender, address(this), _tokenAmount);
        uint256 afterAmount = IERC20(bonusToken).balanceOf(address(this));
        uint256 balance = afterAmount.sub(beforeAmount);
        require(balance == _tokenAmount, "Error balance");
        require(balance > 0 && (cycle * _newPerBlock) <= balance, "Insufficient balance");
        bonusPerBlock = _newPerBlock;
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            ...
            if (pendingAmount > 0) {
                safeTransfer(msg.sender, pendingAmount);
            }
        }
        if (_amount > 0) {
    ...
    function withdraw(uint256 _pid, uint256 _amount) public {
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][msg.sender];
        require(user.amount >= _amount, "Withdrawal not available");
        updatePool(_pid);
        uint256 pendingAmount = user.amount.mul(pool.accBonusPerShare).div(1e12).sub(user.rewardDebt);
        if (pendingAmount > 0) {
            safeTransfer(msg.sender, pendingAmount);
    ...
    function safeTransfer(address _to, uint256 _amount) internal {
        uint256 bal = IERC20(bonusToken).balanceOf(address(this));
        if (_amount > bal) {
            IERC20(bonusToken).transfer(_to, bal);
        } else {
            IERC20(bonusToken).transfer(_to, _amount);
        }
    }
}
