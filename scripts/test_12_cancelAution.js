...
//   address _token,uint _tokenid,uint _offer_idx,address _token_to
  await wmarket.cancelOrWithdrawAuctionOnFixPrice(addrs.mtt,3,1,accounts[1].address);

  
  const market_logs = await ethers.provider.getLogs({
    address:addrs.wmarket,
    topics: [ethers.utils.id("OnAuctionByPriceCanceled(address,uint,uint,address)")]
  });
  market_logs.map(log => {
    console.log("OnAuctionByPriceCanceled=="+JSON.stringify(log));
  })
